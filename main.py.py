...
from visitor_tracking import init_visitors_db, log_visit, get_visitor_stats, get_daily_stats, cleanup_old_visits
...
def _lazy_import_docx():
    ...
    try:
        ...
        DOCX_AVAILABLE = True
        app.logger.info("✅ python-docx successfully imported")
    except ImportError as e:
        app.logger.error(f"❌ Failed to import python-docx: {str(e)}")
        DOCX_AVAILABLE = False
    except Exception as e:
        app.logger.error(f"❌ Unexpected error importing python-docx: {str(e)}")
        DOCX_AVAILABLE = False
...
CONTACT_EMAIL = os.getenv("CONTACT_EMAIL", "hello@eduripple.co.ke")

def send_email_via_brevo(to_email, subject, html_body, from_email=None):
    """Send email using Brevo SMTP"""
    if not BREVO_SMTP_USER or not BREVO_SMTP_PASSWORD:
        app.logger.warning("❌ Brevo credentials not configured. Email not sent.")
        return False
    
    if from_email is None:
        from_email = BREVO_SMTP_USER
    ...
    try:
        ...
        app.logger.info(f"✅ Email sent to {to_email} via Brevo")
        return True
    except Exception as e:
        app.logger.error(f"❌ Error sending email via Brevo: {str(e)}")
        return False
...
try:
    init_visitors_db()
    app.logger.info("✅ Visitor tracking initialized")
except Exception as e:
...
def inject_ga_id():
    """Inject Google Analytics ID into templates."""
...
def trust_proxy_headers():
    ...
    This ensures sessions work properly when the app is behind a proxy/load balancer.
    Also logs visitor data for analytics.
    """
    # Check if we're behind a proxy that's using HTTPS
    if request.headers.get('X-Forwarded-Proto') == 'https':
        # This tells Flask that the connection is HTTPS even though it sees HTTP locally
        request.environ['wsgi.url_scheme'] = 'https'
    
    # Log page visit (excluding health checks and static files)
    if not request.path.startswith('/static') and request.path != '/health':
        try:
            # Store visit info for logging in after_request
            g.visit_logged = False
        except Exception as e:
...
def add_security_headers(response):
    ...
    # Log visitor data (after successful response)
    if not request.path.startswith('/static') and request.path != '/health':
        try:
            user_id = session.get('user_id') if session else None
            session_id = request.cookies.get('session') if request.cookies else None
            log_visit(
                path=request.path,
            ...
            app.logger.debug(f"Error logging visit: {str(e)}")
...
def init_teachers_db():
    ...
    try:
        ...
        if "tenant_id" not in columns:
            conn.execute("ALTER TABLE teachers ADD COLUMN tenant_id TEXT")
...
def get_teacher_by_email(email):
    ...
    try:
        row = conn.execute(
            "SELECT id, full_name, email, school, tenant_id, role, password_hash, subject_area, grade_level, years_experience, bio FROM teachers WHERE email = ?",
            (email.lower().strip(),),
        ).fetchone()
...
def get_teacher_by_id(teacher_id):
    ...
    try:
        row = conn.execute(
            "SELECT id, full_name, email, school, tenant_id, role, password_hash, subject_area, grade_level, years_experience, bio FROM teachers WHERE id = ?",
            (teacher_id,),
        ).fetchone()
        return dict(row) if row else None
    finally:
...
def create_teacher(full_name, email, school, password, subject_area=None, grade_level=None, years_experience=None, bio=None, role="teacher"):
    tenant_id = (school or "").strip().lower() or None
    conn = sqlite3.connect(TEACHERS_DB)
    try:
        conn.execute(
            "INSERT INTO teachers (full_name, email, school, tenant_id, role, password_hash, created_at, subject_area, grade_level, years_experience, bio) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                full_name.strip(),
                email.lower().strip(),
                (school or "").strip(),
...
def get_current_teacher():
    if not session.get("teacher_email"):
        return None
    return get_teacher_by_email(session.get("teacher_email"))


def is_teacher_admin():
    teacher = get_current_teacher()
    if teacher and teacher.get("role") == "admin":
        return True
    admin_email = os.getenv("ADMIN_EMAIL", "admin@example.com")
    return teacher and teacher.get("email") == admin_email


def log_audit_event(action, resource=None, details=None, success=True, user_id=None, user_email=None, user_role=None):
    conn = sqlite3.connect(TEACHERS_DB)
    try:
        ip_address = "unknown"
        try:
            ...
            pass
        conn.execute(
            "INSERT INTO audit_logs (created_at, user_id, user_email, user_role, action, resource, details, ip_address, success) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                datetime.utcnow().isoformat(),
                user_id,
                user_email,
...
def roles_required(*allowed_roles):
    def decorator(route_fn):
        ...
        def wrapper(*args, **kwargs):
            teacher = get_current_teacher()
            if not teacher or teacher.get("role") not in allowed_roles:
                log_audit_event(
                    action="unauthorized_access",
                    resource=request.path if request else None,
                    details=f"required={allowed_roles}",
                    success=False,
                    user_id=teacher.get("id") if teacher else None,
                    user_email=teacher.get("email") if teacher else None,
                    user_role=teacher.get("role") if teacher else None,
                )
...
def save_to_word(file_path, content):
    """Save content to Word document with proper formatting"""
    _lazy_import_docx()
    ...
    if DOCX_AVAILABLE and Document:
        try:
            ...
            style.font.size = 12 * 1048576  # 12pt
            ...
            for line in content.split("\n"):
                ...
                if line:
                    ...
                    if line.isupper() or line.startswith(('LESSON PLAN', 'SCHEME OF WORK', 'RUBRIC', '##', '###')):
                        p = doc.add_paragraph(line, style='Heading 1')
                    else:
                        doc.add_paragraph(line)
            
            doc.save(file_path)
            app.logger.info(f"✅ Successfully created Word document: {file_path}")
            return True
        except Exception as e:
            app.logger.error(f"❌ Error creating Word document: {str(e)}")
            traceback.print_exc()
    
    # If we reach here, python-docx is not available or failed
    # Try to create a minimal .docx file manually (Office Open XML format)
    try:
        import zipfile
        import xml.etree.ElementTree as ET
        
        # Create minimal Word document structure
        _create_minimal_docx(file_path, content)
        app.logger.info(f"✅ Created minimal Word document (fallback): {file_path}")
        return True
    except Exception as e:
        app.logger.error(f"❌ Failed to create minimal Word document: {str(e)}")
        # Final fallback: save as text file with .txt extension
        txt_path = file_path.replace('.docx', '.txt')
        with open(txt_path, "w", encoding="utf-8") as f:
            f.write(content)
        app.logger.warning(f"⚠️ Saved as text file instead: {txt_path}")
        return False


def _create_minimal_docx(file_path, content):
    """Create a minimal .docx file (ZIP with XML) when python-docx is not available"""
    import zipfile
    import xml.etree.ElementTree as ET
...
def create_download_files(content, intent, prompt):
    ...
    return {
        ...
        "docx": f"/resources/{word_filename}",
...
def analytics_daily(date_str):
    """Get stats for a specific day (format: YYYY-MM-DD)."""
    try:
        stats = get_daily_stats(date_str=date_str)
        return jsonify(stats), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def analytics_cleanup():
    """Remove old visitor records (admin only)."""
    # Check if admin/authorized (you can add proper auth)
    api_key = request.headers.get('X-API-Key')
    expected_key = os.getenv('ADMIN_API_KEY', 'change-me')
    ...
    try:
        deleted = cleanup_old_visits(days=days)
        return jsonify({
            "success": True,
            "message": f"Deleted {deleted} visitor records older than {days} days"
        }), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def sitemap_xml():
    ...
    for rule in sorted(app.url_map.iter_rules(), key=lambda r: r.rule):
        if "GET" not in rule.methods or rule.arguments:
            continue
        if rule.rule.startswith("/static") or rule.rule.startswith("/api") or rule.rule.startswith("/admin"):
            continue
        sitemap_urls.append(base_url + rule.rule)

    xml_urls = "\n".join(
        f"  <url><loc>{url}</loc></url>" for url in sitemap_urls
    )
...
def submit_contact():
    ...
    email_subject = f"New Contact: {inquiry_type} from {name}"
    email_html = f"""
    <html>
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; border: 1px solid #ddd; border-radius: 8px; padding: 20px; background: #f9f9f9;">
                <h2 style="color: #0066cc; margin-bottom: 20px;">🎓 New Contact Form Submission</h2>
                
                <div style="background: white; padding: 15px; border-radius: 5px; margin-bottom: 15px;">
                    <p><strong>Name:</strong> {name}</p>
                    <p><strong>Email:</strong> <a href="mailto:{email}">{email}</a></p>
                    <p><strong>Phone:</strong> {phone if phone else 'Not provided'}</p>
                    <p><strong>School:</strong> {school if school else 'Not provided'}</p>
                    <p><strong>Inquiry Type:</strong> <span style="background: #0066cc; color: white; padding: 5px 10px; border-radius: 3px;">{inquiry_type}</span></p>
                </div>
                
                <div style="background: white; padding: 15px; border-radius: 5px; margin-bottom: 15px; border-left: 4px solid #0066cc;">
                    <h3 style="margin-top: 0; color: #0066cc;">Message:</h3>
                    <p>{message}</p>
                </div>
                
                <div style="background: #f0f0f0; padding: 10px; border-radius: 5px; font-size: 0.9em; color: #666;">
                    <p><strong>To reply:</strong> Click reply and respond to {email}</p>
    ...
    send_email_via_brevo(CONTACT_EMAIL, email_subject, email_html)
...
@limiter.limit("5 per hour")  # Prevent account spam
def teacher_signup():
    ...
    create_teacher(full_name, email, school, password, subject_area, grade_level, years_exp_int, role="teacher")
    teacher = get_teacher_by_email(email)
    if teacher:
        log_audit_event(
            action="signup_success",
            resource="/teacher/signup",
            details=f"email={email}",
            success=True,
            user_id=teacher["id"],
            user_email=teacher["email"],
            user_role=teacher.get("role"),
        )
...
def teacher_signout():
    ...
    if teacher:
        log_audit_event(
            action="signout",
            resource="/teacher/signout",
            details=f"email={teacher['email']}",
            success=True,
            user_id=teacher["id"],
            user_email=teacher["email"],
            user_role=teacher.get("role"),
...
def format_scheme_of_work(response, structured_info=None, prompt=""):
    ...
    return f"""SCHEME OF WORK
================================================================================

School: ____________________     Teacher: ____________________
Subject: {subject_text.upper()}     Grade: {grade_text}     Term: {term_text}     Year: {current_year}
Approved by HoD: _____________  Sign: ___________  Date: ___________

| Week | Lesson No. | Strand | Sub-Strand | Specific Learning Outcomes | Learning Experiences / Activities | Key Inquiry Question(s) | Learning Resources | Assessment Methods | Core Competencies | Values | PCIs (Pertinent & Contemporary Issues) | Remarks |
| :--: | :--: | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | 1 | {strand_text} | {substrand_text} | • By the end of the lesson, the learner should be able to: a) __________________ b) __________________ c) __________________ | • Learners are guided to: __________________ | • __________________ | • Learner's Book • Teacher's Guide • Charts • Digital/print resources | • Observation • Oral questions • Written exercise | Critical thinking and problem solving, Communication and collaboration, Self-efficacy | Respect, Responsibility, Unity | Digital Literacy — using digital platforms for reading and written communication | |
|  | 2 | {strand_text} | {substrand_text} | • By the end of the lesson, the learner should be able to: a) __________________ b) __________________ c) __________________ | • Learners are guided to: __________________ | • __________________ | • Learner's Book • Teacher's Guide • Local materials | • Observation • Oral questions • Written exercise | Critical thinking and problem solving, Communication and collaboration, Self-efficacy | Respect, Responsibility, Unity | Digital Literacy — using digital platforms for reading and written communication | |
| 2 | 3 | {strand_text} | {substrand_text} | • By the end of the lesson, the learner should be able to: a) __________________ b) __________________ c) __________________ | • Learners are guided to: __________________ | • __________________ | • Textbook • Board • Worksheets | • Observation • Oral questions • Written exercise | Critical thinking and problem solving, Communication and collaboration, Self-efficacy | Respect, Responsibility, Unity | Digital Literacy — using digital platforms for reading and written communication | |
...
def build_offline_scheme_of_work(prompt, structured_info):
    ...
    weekly_rows = [
        f"| 1 | 1 | {strand_text} | {substrand_text} | • By the end of the lesson, the learner should be able to: a) describe key concepts in {substrand_text}; b) identify practical applications; c) apply learning in class task. | • How can this topic improve classroom and community practice? | • Learners are guided to: discuss key ideas in groups and complete guided practical activity | • Sample materials • Teacher's Guide • Learner's Book | • Observation • Written tests and assignments • Oral assessment | Critical thinking and problem solving, Communication and collaboration, Self-efficacy | Respect, Responsibility, Unity | Digital Literacy — using digital platforms for reading and written communication | |",
        f"|  | 2 | {strand_text} | {substrand_text} | • By the end of the lesson, the learner should be able to: a) explain class activity steps; b) use available resources effectively; c) demonstrate learned skill. | • What steps are needed to complete the task successfully? | • Learners are guided to: research using digital/print resources and present findings in pairs | • Charts • Textbook • Local resources | • Observation • Projects • Oral assessment | Critical thinking and problem solving, Communication and collaboration, Self-efficacy | Respect, Responsibility, Unity | Digital Literacy — using digital platforms for reading and written communication | |",
        f"|  | 3 | {strand_text} | {substrand_text} | • By the end of the lesson, the learner should be able to: a) perform task independently; b) evaluate outcomes; c) suggest improvements. | • How can we improve outcomes using local resources? | • Learners are guided to: perform practical class task and reflect on results and improvements | • Worksheets • Board • Reference notes | • Written tests • Assignments • Practical task | Critical thinking and problem solving, Communication and collaboration, Self-efficacy | Respect, Responsibility, Unity | Digital Literacy — using digital platforms for reading and written communication | |",
        f"| 2 | 4 | {strand_text} | {substrand_text} | • By the end of the lesson, the learner should be able to: a) extend prior knowledge; b) collaborate in teams; c) apply correct procedures. | • Which methods are most effective for this topic? | • Learners are guided to: complete pair activity and document outcomes in exercise books | • Learner's Book • Teacher notes • Sample tools | • Observation • Oral assessment | Critical thinking and problem solving, Communication and collaboration, Self-efficacy | Respect, Responsibility, Unity | Digital Literacy — using digital platforms for reading and written communication | |",
        f"|  | 5 | {strand_text} | {substrand_text} | • By the end of the lesson, the learner should be able to: a) solve task-based challenges; b) interpret findings; c) report results clearly. | • How do we measure success in this learning area? | • Learners are guided to: analyze outcomes and share reports and receive feedback | • Charts • Worksheets • Digital resources | • Written tests and assignments • Projects | Critical thinking and problem solving, Communication and collaboration, Self-efficacy | Respect, Responsibility, Unity | Digital Literacy — using digital platforms for reading and written communication | |",
        f"|  | 6 | {strand_text} | {substrand_text} | • By the end of the lesson, the learner should be able to: a) consolidate the week's learning; b) demonstrate competence; c) reflect on strengths and gaps. | • What should we improve in the next lessons? | • Learners are guided to: do recap tasks and complete reflection questions | • Revision materials • Board • Textbook | • Observation • Oral assessment • Short test | Critical thinking and problem solving, Communication and collaboration, Self-efficacy | Respect, Responsibility, Unity | Digital Literacy — using digital platforms for reading and written communication | |"
    ]

    return (
        "SCHEME OF WORK\n"
        "================================================================================\n\n"
        f"School: ____________________     Teacher: ____________________\n"
        f"Subject: {subject_text.upper()}     Grade: {grade_text}     Term: {term_text}     Year: {current_year}\n"
        f"Approved by HoD: _____________  Sign: ___________  Date: ___________\n\n"
        "| Week | Lesson No. | Strand | Sub-Strand | Specific Learning Outcomes | Learning Experiences / Activities | Key Inquiry Question(s) | Learning Resources | Assessment Methods | Core Competencies | Values | PCIs (Pertinent & Contemporary Issues) | Remarks |\n"
        "| :--: | :--: | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |\n"
        + "\n".join(weekly_rows)
        + "\n\nTeacher Reflection Notes:\n"
...
def download_resource(filename):
    ...
    return send_from_directory(RESOURCE_DIR, filename, as_attachment=True, mimetype=mimetype)
...
@login_required_json
@limiter.limit("20 per hour")  # Audio generation is resource-intensive
def generate_audio():
    """
...
@login_required_json
@limiter.limit("20 per hour")  # Flashcard generation
...
@login_required_json
@limiter.limit("10 per hour")  # Video generation is very resource-intensive
def generate_video():
    """
...
def _is_admin():
    """Check if current user has admin privileges based on the teacher role."""
    teacher = get_current_teacher()
    return is_teacher_admin(teacher)
